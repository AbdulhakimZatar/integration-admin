...
class Kubernetes {
  ...
  public async updateService(oldName: string, newName: string, namespace: string) {
    const service = {
      apiVersion: 'v1',
      kind: 'Service',
      metadata: {
    ...
    return this.coreV1Api.patchNamespacedService(oldName, namespace, service).then((res) => res.body);
  }

  public async updateDeployment(oldName: string, newName: string, namespace: string) {
    const deployment = {
      apiVersion: 'apps/v1',
      kind: 'Deployment',
      metadata: {
    ...
    return this.appsV1Api.patchNamespacedDeployment(oldName, namespace, deployment).then((res) => res.body);
  }
